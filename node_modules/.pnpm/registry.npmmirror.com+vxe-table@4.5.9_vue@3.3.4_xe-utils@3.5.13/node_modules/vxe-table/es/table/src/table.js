...
export default defineComponent({
    ...
    setup: function (props, context) {
        ...
        var handleAsyncRowExpand = function (row) {
            var fullAllDataRowIdData = internalData.fullAllDataRowIdData;
            var rowExpandLazyLoadedMaps = reactData.rowExpandLazyLoadedMaps, rowExpandedMaps = reactData.rowExpandedMaps;
            return new Promise(function (resolve) {
                var expandOpts = computeExpandOpts.value;
                ...
                if (loadMethod) {
                    ...
                    rowExpandLazyLoadedMaps[rowid_1] = row;
                    loadMethod({ $table: $xetable, row: row, rowIndex: tableMethods.getRowIndex(row), $rowIndex: tableMethods.getVMRowIndex(row) }).then(function () {
                        ...
                        rowExpandedMaps[rowid_1] = row;
                    }).catch(function () {
                        rest_1.expandLoaded = false;
                    }).finally(function () {
                        if (rowExpandLazyLoadedMaps[rowid_1]) {
                            delete rowExpandLazyLoadedMaps[rowid_1];
                        }
                        nextTick().then(function () { return tableMethods.recalculate(); }).then(function () { return resolve(); });
                    });
                }
            ...
            loadTreeChildren: function (row, childRecords) {
                ...
                return tableMethods.createData(childRecords).then(function (rows) {
                    if (keepSource) {
                        ...
                        var matchObj = XEUtils.findTree(tableSourceData, function (item) { return rowid_2 === getRowid($xetable, item); }, { children: childrenField });
                        if (matchObj) {
            ...
            getRadioReserveRecord: function (isFull) {
                ...
                if (radioOpts.reserve && radioReserveRow) {
                    ...
                    if (isFull) {
                        ...
                        if (treeConfig) {
                            var matchObj = XEUtils.findTree(afterFullData, function (row) { return rowid_3 === XEUtils.get(row, rowkey_2); }, { children: childrenField });
                            if (matchObj) {
                                return radioReserveRow;
                            }
                        }
        ...
        onMounted(function () {
            nextTick(function () {
                ...
                if (props.autoResize) {
                    ...
                    resizeObserver = createResizeEvent(resizeOpts.refreshDelay ? XEUtils.throttle(function () {
                        tableMethods.recalculate(true);
