  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (message == null || message == undefined) {
      if (Reflect.has(data, 'msg')) {
        message = data.msg;
      } else if (Reflect.has(data, 'errorMsg')) {
        message = data.errorMsg;
      }
    }
    if (code == null || code == undefined) {
      if (Reflect.has(data, 'errorCode')) {
        code = data.code = data.errorCode == ResultEnum.OK ? ResultEnum.SUCCESS : data.errorCode;
      }
    }
    // ************************** AJAX请求返回结果适配 **************************
  ...
  requestInterceptors: (config, options) => {
    ...
    if (token && (config as Recordable)?.requestOptions?.withToken !== false) {
      ...
      if (config.url?.startsWith('/admin-api/baseset')) {
        (config as Recordable).headers.version = '1.0.0';
      }
